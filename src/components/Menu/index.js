...
import { setActivite } from '../../store/modules/takeaway'
import './index.scss'
...
const Menu = () => {
 ...
 const {foodsList,activityIndex} = useSelector(state=>state.foods);
  ...
  return (
    <nav className="list-menu">
      ...
      {menus.map((item, index) => {
        return (
          <div
            onClick={()=>dispatch(setActivite(index))}
            key={item.tag}
            className={classNames(
              'list-menu-item',
              activityIndex === index && 'active'
            )}
          >
            {item.name}
          </div>
